...
- **庫存異動歷史**：日期篩選欄新增「今天 / 本週 / 本月 / 上月 / 近90天」快選按鈕，與銷售/採購頁體驗一致
...
- **表格欄位自訂顯示全面展開**：商品/銷售/採購/客戶/供應商頁面均可透過右上角齒輪圖示隱藏不需要的欄位，選擇持久化至 localStorage
...
- **客戶 / 供應商批次操作**：兩頁面新增 checkbox 多選 + 底部批次操作列，支援批次刪除與選取匯出 CSV
...
- **快速鍵面板強化**：面板（按 `?` 開啟）動態反映自訂 G+ 導覽鍵、展示全部 11 條路由、新增目前頁面情境操作區塊與所在頁高亮、底部新增「自訂快捷鍵」捷徑按鈕
...
- **客戶積分系統**：銷售單完成時自動依金額累積點數（可設定匯率），支援退貨自動扣點、手動調整點數，客戶詳情頁新增點數餘額顯示與點數記錄 Tab
...
- **採購訂單範本**：在建立採購單表單新增「另存為範本」與「載入範本」功能，可將常用的供應商 + 品項組合儲存為範本，下次一鍵帶入後視需要調整再送出；範本清單支援刪除操作
...
- **商品標籤列印**：單品 Tag 按鈕 + 批次列印標籤，輸出含 QR Code（編碼 SKU）、商品名、售價的 47.5×30mm 貼紙 PDF（A4 每頁 ~36 張）
...
- **封面頁**：匯出 Word 文件首頁自動生成導出日期、對話輪數（問 / 答數量）、平均忠實度與評級
- **每答 Metadata 行**：每個 AI 回答上方新增淺灰 metadata 列，顯示時間戳、模型名稱、忠實度分數、token 數量
- **引用來源與延伸問題**：每個 AI 回答後附上引用來源清單與 follow-up 建議問題
...
- **RAG 品質儀表板強化**：新增忠實度 14 天趨勢折線圖、實體查詢熱點排行（Top 5 商品/客戶/供應商 + avg faithfulness badge）、複合問題拆解統計、低分問答 Review 清單（faithfulness < 70 可點擊展開）
...
- **複合問題拆解（Query Decomposition）**：偵測含「和」「各」「分別」等訊號詞的複合問題，自動拆解為獨立子問題並行檢索，各自取回對應業務資料後以標記區段合併；LLM 收到結構清晰的分段 context，回答更準確；不屬複合問題時自動退回單一查詢路徑
...
- **即時強化學習（Adaptive Retrieval via Entity Feedback）**：新增 `ai_entity_stats` 表追蹤每個商品／客戶／供應商的查詢次數與 faithfulness 累計分數；當同一實體被查詢 ≥ 2 次且平均分數 < 70 時，系統自動切換擴展模式，補充近 90 天銷售軌跡、最近 10 筆訂單／採購明細，形成低分 → 擴展 → 改善的自我優化迴路
...
- **結構化引用標記**：AI 回答中每個具體數字後自動嵌入 `[銷售]`/`[庫存]`/`[客戶]`/`[供應商]` 色彩 badge，讓數字來源可追溯；同時修正歷史訊息底部引用徽章空白問題
...
- **跨回合實體記憶**：對話中使用代詞（他、那個供應商、同樣的商品）時，自動從 session 歷史補充實體，無需使用者重複說明；純 DB 查詢，零 API 額外消耗
...
- **自適應 Context 剪裁**：Context 超過 2,500 字元時以 Haiku 自動剪裁至最相關條目，
  避免多主題 + 多實體命中時過量資料稀釋回答焦點；正常查詢零額外消耗
...
- **macOS 更新安裝失敗（根本解決）**：macOS 改為偵測新版本後顯示「前往下載頁面」按鈕，開啟 GitHub Releases 讓使用者手動下載 DMG，完全繞開 ShipIt 程式碼簽名驗證問題
...
- **語意實體匹配**：LIKE 全部失配時啟動 Haiku 消歧義 fallback，比對資料庫實際實體清單（支援縮寫、別名、拼字相似），對應結果持久化為 alias 快取，下次相同縮寫零 API 消耗直接命中
...
- **結構化多輪摘要**：超過 8 則對話時，以 JSON 格式（涉及實體、已確認事實、待確認事項）摘要舊訊息，取代原本自由文字
- **摘要持久化快取**：摘要寫入 DB，相同舊訊息不重複呼叫 Haiku；token 預算 200 → 400
...
- **自動更新提示修正**：開發模式下按「檢查更新」原本會誤顯示「已是最新版本」，現已改為明確提示開發模式不支援自動更新
