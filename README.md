...
- **CSV 匯出**：一鍵匯出全部商品清單
- **商品標籤列印**：每列新增 Tag 圖示按鈕，生成含 QR Code（編碼 SKU）、商品名、SKU、售價的 47.5×30mm 貼紙 PDF（A4 每頁 ~36 張）；支援批次多選後一次列印多張標籤
...
- **訂單範本**：常用供應商 + 品項組合可另存為範本，下次建立採購單時一鍵載入並帶入供應商、品項、數量、單價、折扣；範本清單支援刪除
...
- **客戶積點**：銷售單完成時自動依金額累積點數（積分匯率可在設定頁調整，預設 NT$100 = 1 點）；退貨時自動扣回對應點數；管理員可手動調整點數（含備註）
- **客戶詳情**：點擊客戶名稱開啟 Dialog，顯示聯絡資訊、信用額度進度條、消費統計、**點數餘額**；新增「點數記錄」Tab，列出最近 50 筆點數變動明細（類型、關聯訂單號、備註）；對帳單 Tab 支援日期區間篩選 + 已付/未付金額統計
...
- **完整 CRUD**：名稱、聯絡人、電話、Email、地址、信用額度
- **批次操作**：多選後浮動操作列提供「CSV 匯出」與「批次刪除」（含二次確認）
- **CSV 匯出**：選取的列以 UTF-8 BOM 格式匯出，欄位含名稱、聯絡人、電話、Email、地址、信用額度
- **欄位自訂顯示**：右上角齒輪圖示可隱藏聯絡人、電話、Email、地址、信用額度五欄，設定以 localStorage 持久化
- **客戶詳情**：消費統計、點數餘額、點數記錄、信用額度進度條、對帳單（含日期區間篩選）
- **供應商詳情**：採購統計、信用額度進度條、對帳單
...
| `?` | 開啟快捷鍵說明面板（動態顯示所有導覽快捷鍵與當前頁面操作） |
| `Esc` | 關閉彈窗 / 快捷鍵面板 |
| `N`（在商品 / 採購 / 銷售 / 客戶 / 供應商 / 盤點頁） | 開啟新增表單 |
| `G` → `H` | 導覽至首頁 Dashboard |
| `G` → `P` | 導覽至商品頁 |
| `G` → `B` | 導覽至採購頁 |
| `G` → `S` | 導覽至銷售頁 |
...
> `?` 快捷鍵面板會即時反映自訂後的按鍵配置，並依當前頁面顯示可用的情境操作（如 `N` 新增）。
...
- **欄位自訂顯示**：各頁面表格右上角齒輪圖示可隱藏次要欄位（商品：分類；銷售：客戶 / 日期；採購：供應商 / 日期；客戶 / 供應商：聯絡人、電話、Email、地址、信用額度），設定各自獨立儲存
