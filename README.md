...
- **採購管理** — 採購單建立、確認收貨（自動更新庫存）、供應商管理、PDF 列印
- **銷售管理** — 銷售單建立、完成銷售（事務性庫存扣除）、建單時即時庫存警示、客戶管理、PDF 列印
- **報表分析** — 銷售趨勢、庫存分佈、產品銷售排行、低庫存警示
- **庫存盤點** — 建立盤點單、逐品項填寫實際庫存、完成後自動調整差異
- **庫存異動歷史** — 完整異動紀錄，支援篩選與 CSV 匯出

### 智慧補貨

- **補貨建議卡片**（Dashboard）：自動列出低於補貨點的商品，勾選後一鍵建立採購單
- **低庫存桌面通知**：App 啟動時 + 確認出貨後若有商品跌破補貨點，立即跳出系統通知
